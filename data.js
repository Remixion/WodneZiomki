...
function champImgUrl(nameOrKey){
  ...
  return key?'https://ddragon.leagueoflegends.com/cdn/15.10.1/img/champion/'+key+'.png':'';
}

// Fetch full champion list from Data Dragon at startup
async function fetchAllChamps(){
  try{
    const r=await fetch('https://ddragon.leagueoflegends.com/cdn/15.10.1/data/en_US/champion.json',{cache:'no-store'});
    if(!r.ok)return;
